...
FUNCTION qmi_idl_message_decode_v5
...
***********************************************************************/
void qmi_idl_message_decode_v5
(
...
)
{
  ...
  /* Top level API function -- validate passed parameters */
  if ( ! p_service ||
       QMI_IDL_NUM_MSG_TYPES <= req_resp_ind ||
       ! p_src ||
       ! p_dst ) {
    QMI_IDL_HANDLE_ERROR( exc, QMI_IDL_LIB_PARAMETER_ERROR, 0, 0, 0 );
  }
...
FUNCTION qmi_idl_message_encode_v4
...
***********************************************************************/
static uint32_t qmi_idl_message_encode_v5
(
  /* Pointer to service object, from auto-generated header from IDL */
  const qmi_idl_service_object_type p_service,
...
)
{
  ...
  case 6:
  case 7:
    qmi_idl_message_decode_v5( p_service,
                               req_resp_ind,
                               message_id,
                               p_src,
                               src_len,
...
)
{
  ...
  case 5:
  case 6:
    *dst_encoded_len = qmi_idl_message_encode_v5( p_service,
                                                  req_resp_ind,
                                                  message_id,
                                                  p_src,
                                                  p_dst,
...
)
{
  ...
  memcpy(p_dst2,&result,1);
  p_dst2 +=2;
  memcpy(p_dst2,&error,1);
