...
#define WDS_MUX_ID_TWWAN0  0x80
#define WDS_MUX_ID_TWWAN1  0x81
#define WDS_MUX_ID_TWWAN2  0x82

unsigned long pkt_data_handle0 = 0;
unsigned long pkt_data_handle1 = 0;
/*
#define WDS_MUX_ID_TWWAN0  0x81
...
static int bind_mux_id(qmi_client_type user_handle, uint8_t mux_id)
{
    int rc;
    wds_bind_mux_data_port_req_msg_v01 req;
    wds_bind_mux_data_port_resp_msg_v01 rsp;
    ...
    req.ep_id_valid = 1;
    req.ep_id.ep_type = 0x02;  //DATA_EP_TYPE_HSUSB
    req.ep_id.iface_id = 0x04; //interface 4 as ndis card.

    req.client_type_valid = 1;
    req.client_type = 0x01; //WDS_CLIENT_TYPE_TETHERED.
    ...
    if (rc == QMI_NO_ERR)
    {
        printf("bind mux id=%x successful.\n", mux_id);
        printf("%d, 0x%x\n", rsp.resp.result, rsp.resp.error);
    }
    else
    {
        printf("start bind mux id=%x failed.\n", mux_id);
        printf("%d, 0x%x\n", rsp.resp.result, rsp.resp.error);
    }
...
static int dial_v4(qmi_client_type user_handle,int profile_index,unsigned long* data_handle)
{
    int rc;
    ...
    req.technology_preference = 0x01; //bit0 as 3GPP, bit1 as 3GPP2
    req.technology_preference_valid = 1;
    req.profile_index_valid = 1;
    req.profile_index = profile_index;
    //   req.enable_autoconnect_valid = 1;
    //   req.enable_autoconnect = 1;
    printf("apn:%s\nuser name:%s\npassword:%s,req.profile_index=%d.\n",
           req.apn_name, req.username, req.password, req.profile_index);
    ...
    if(rc == QMI_NO_ERR)
    {
        ...
        {
            *data_handle = rsp.pkt_data_handle;
        }
    }
...
static int stop_v4(qmi_client_type user_handle, unsigned long data_handle)
{
    int rc;
    ...
    memset(&req,0,sizeof(req));
    memset(&rsp,0,sizeof(rsp));
    req.pkt_data_handle = data_handle;
...
int main(int argc, char *argv[])
{
    ...
    qmi_client_type wds_user_handle_twwan0;
    qmi_client_type wds_user_handle_twwan1;
    int option;
    int run = 1;
  ...
  printf("PING: qmi_client_get_service_list() returned %d num_entries = %d num_services = %d\n", rc, num_entries, num_services);
   ...
   /* Initialize a connection to first QMI control port */
    ...
    /********************************************************
     * start bind muxID port by wds client.
     * *****************************************************/
    rc = bind_mux_id(wds_user_handle_twwan0, WDS_MUX_ID_TWWAN0);
    if (rc != QMI_NO_ERR)
    {
        printf("bind mux id failed...Error Code:%d\n", rc);
        return 0;
    }
    rc = bind_mux_id(wds_user_handle_twwan1, WDS_MUX_ID_TWWAN1);
    if (rc != QMI_NO_ERR)
    {
        printf("bind mux id failed...Error Code:%d\n", rc);
        return 0;
    ...
    while(run)
    {
        ...
        printf("24.dial v4 of profile 1 over twwan0\n");
        printf("25.stop v4 of profile 1 over twwan0\n");
        ...
        printf("34.dail v4 of profile 2 over twwan1\n");
        printf("35.stop v4 of profile 2 over twwan1\n");
        ...
        switch(option)
        {
            ...
            case 24:
            {
                rc = dial_v4(wds_user_handle_twwan0, 1, &pkt_data_handle0);
               // system("ifconfig twwan0 up;udhcpc -i twwan0");
                break;
            }
            case 25:
            {
                rc = stop_v4(wds_user_handle_twwan0, pkt_data_handle0);
              //  system("ifconfig twwan0 down");
                break;
            }
            case 26:
            {
                rc = get_ipinfo_bywds(wds_user_handle_twwan0);
                break;
            }
            case 27:
            {
                rc = get_profile(wds_user_handle_twwan0);
                break;
            }
            case 28:
            {
                rc = create_profile(wds_user_handle_twwan0);
                break;
            }
            case 29:
            {
                rc = modify_profile(wds_user_handle_twwan0);
                break;
            }
            case 30:
            {
            ...
            case 34:
            {
                rc = dial_v4(wds_user_handle_twwan1,2,&pkt_data_handle1);
              //  system("ifconfig twwan1 up;udhcpc -i twwan1");
                break;
            }
            case 35:
            {
                rc = stop_v4(wds_user_handle_twwan1, pkt_data_handle1);
              //  system("ifconfig twwan1 down");
                break;
            }
            case 36:
            {
                rc = get_ipinfo_bywds(wds_user_handle_twwan1);
                break;
            }
            case -1:
            {
...

    {
        ...
        rc = qmi_client_release(wds_user_handle_twwan0);
        if (rc < 0 ) {
            printf("Release not successful \n");
        }else
        {
        ...
        rc = qmi_client_release(wds_user_handle_twwan1);
        if (rc < 0 ) {
            printf("Release not successful \n");
        }else
        {
